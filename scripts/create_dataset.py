...
def main():
    ...
    parser.add_argument("--origin-path", default="./libero/datasets/libero_spatial")
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default="outputs/datasets/libero_spatial",
    )
    ...
    for filename in os.listdir(args.origin_path):
        # 拼接完整的文件路径
        hdf5_path = os.path.join(args.origin_path, filename)
        # 如果是文件并且后缀是 .hdf5
        if not (os.path.isfile(hdf5_path) and filename.endswith('.hdf5')):
            continue

        print_green(f"Process file {filename}")
        ...
        h5py_f = h5py.File(output_file, "w", locking=False)
        ...
        for (i, ep) in enumerate(demos):
            ...
            agentview_ex_paras = []
            agentview_in_paras = []
            eye_in_hand_ex_paras = []
            eye_in_hand_in_paras = []
            ...
            for j, action in enumerate(actions):
                ...
                if args.use_camera_obs:
                    ...
                    agentview_ex_paras.append(camera_utils.get_camera_extrinsic_matrix(env.sim, "agentview"))
                    agentview_in_paras.append(camera_utils.get_camera_intrinsic_matrix(env.sim, "agentview", 128, 128))
                    eye_in_hand_ex_paras.append(camera_utils.get_camera_extrinsic_matrix(env.sim, "robot0_eye_in_hand"))
                    eye_in_hand_in_paras.append(camera_utils.get_camera_intrinsic_matrix(env.sim, "robot0_eye_in_hand", 128, 128))

                    print_green(camera_utils.get_camera_intrinsic_matrix(env.sim, "agentview", 128, 128))
                    print_green(camera_utils.get_camera_intrinsic_matrix(env.sim, "agentview", 256, 256))
                    os.exit(1)
            ...
            obs_grp.create_dataset("agentview_ex_paras", data=np.stack(agentview_ex_paras, axis=0))
            obs_grp.create_dataset("agentview_in_paras", data=np.stack(agentview_in_paras, axis=0))
            obs_grp.create_dataset("eye_in_hand_ex_paras", data=np.stack(eye_in_hand_ex_paras, axis=0))
            obs_grp.create_dataset("eye_in_hand_in_paras", data=np.stack(eye_in_hand_in_paras, axis=0))
        ...
        print_green(f"The created dataset is saved in the following path: {output_file}")
